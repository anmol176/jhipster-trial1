...
export default class CountryUpdatePage {
  pageTitle: ElementFinder = element(by.id('trial1App.country.home.createOrEditLabel'));
  saveButton: ElementFinder = element(by.id('save-entity'));
  cancelButton: ElementFinder = element(by.id('cancel-save'));
  countryNameInput: ElementFinder = element(by.css('input#country-countryName'));
  isoCodeInput: ElementFinder = element(by.css('input#country-isoCode'));
  ...
  async setIsoCodeInput(isoCode) {
    await this.isoCodeInput.sendKeys(isoCode);
  }

  async getIsoCodeInput() {
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setIsoCodeInput('isoCode');
    expect(await this.getIsoCodeInput()).to.match(/isoCode/);
