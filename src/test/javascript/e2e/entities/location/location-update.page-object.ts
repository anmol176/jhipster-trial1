...
export default class LocationUpdatePage {
  pageTitle: ElementFinder = element(by.id('trial1App.location.home.createOrEditLabel'));
  ...
  locationDetailInput: ElementFinder = element(by.css('input#location-locationDetail'));
  ...
  async setLocationDetailInput(locationDetail) {
    await this.locationDetailInput.sendKeys(locationDetail);
  }

  async getLocationDetailInput() {
    return this.locationDetailInput.getAttribute('value');
  }
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setLocationDetailInput('locationDetail');
    expect(await this.getLocationDetailInput()).to.match(/locationDetail/);
